<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h2 class="text-3xl font-bold text-white tracking-tight mb-2">Pengalaman Karir</h2>
        <p class="text-zinc-400">Kelola riwayat pekerjaan dan pengalaman organisasi Anda.</p>
      </div>
        ...
        <table class="w-full text-left text-sm text-zinc-400">
          <thead class="text-xs text-zinc-500 uppercase bg-zinc-950/50 border-b border-white/5">
            <tr>
              ...
              <th scope="col" class="px-6 py-4 font-medium">Perusahaan / Organisasi</th>
          ...
          <tbody class="divide-y divide-white/5">
            <tr v-if="experiences.length === 0">
              <td colspan="4" class="px-6 py-8 text-center text-zinc-500">Belum ada riwayat pengalaman.</td>
            </tr>
             <tr v-for="item in experiences" :key="item.id" class="hover:bg-zinc-800/50 transition-colors">
              <td class="px-6 py-4">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div class="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="p-6 border-b border-white/5 flex justify-between items-center sticky top-0 bg-zinc-900 z-10">
          <h3 class="text-xl font-bold text-white">{{ isEditing ? 'Edit Riwayat' : 'Tambah Riwayat Karir' }}</h3>
        ...
        <form @submit.prevent="saveItem" class="p-6 space-y-5">
          <div>
            <label class="block text-sm font-medium text-zinc-400 mb-1.5">Posisi / Pekerjaan</label>
            <input v-model="form.role" type="text" required class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-zinc-200 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-all" />
          </div>
          <div>
            <label class="block text-sm font-medium text-zinc-400 mb-1.5">Perusahaan / Organisasi</label>
          ...
          <div>
            ...
            <input v-model="form.year" type="text" placeholder="Misal: 2025 - Sekarang" required class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-zinc-200 focus:border-amber-500 outline-none transition-all" />
          </div>
          <div>
...
const saveItem = async () => {
  ...
  try {
    ...
    let result
    if (isEditing.value) {
      result = await supabase.from('experiences').update(payload).eq('id', form.value.id)
    } else {
